...
$(function () {
    ...
    // recalculer w au window resize et redonner un widt aux li
    ...
    // défilement vers right 
    let defilRight = function () {
        $('.slider ul').css({
            'left': -w
        });
        $('.slider li:first').before($('.slider ul li:last'));
        $('.slider ul').animate({
            'left': 0
        }, 1000, function () {
            auth = true;
    ...
    //  quand on quitte un boutton, on relance le défilement
    $('.slider span').mouseleave(function () {
        id = setInterval(defilLeft, 3000);
    });

    // quand on clique sur btn before
